...
function SpotlightOverlay() {
  ...
  const computeTooltipPosition = useCallback(
    (rect: Rect, position: TourStep["position"]) => {
      ...
      // Clamp vertical — ensure tooltip stays within viewport
  ...
  const measureAndSet = useCallback(
    (el: HTMLElement) => {
      ...
      setTargetRect(rect);
      // Delay position calc slightly to allow tooltip to render & measure
  ...
  // ── re-calc position after tooltip renders (for accurate height) ──
  useEffect(() => {
  ...
  return createPortal(
    <>
      ...
      {/* Tooltip Card — fixed position, clamped to viewport */}
