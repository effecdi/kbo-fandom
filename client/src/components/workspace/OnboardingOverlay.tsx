...
import { getOnboardingPrompts, TEMPLATE_LABELS } from "@/lib/fandom-templates";
...
export function OnboardingOverlay() {
  ...
  useEffect(() => {
    // Show onboarding for both fandom and non-fandom mode
  ...
  const prompts = fandomMeta
    ? getOnboardingPrompts(fandomMeta)
    : EXAMPLE_PROMPTS;

  const templateLabel = fandomMeta
    ...
    <div className="fixed inset-0 z-40 pointer-events-none flex flex-col items-center justify-center">
      ...
      <div className="relative z-10 text-center pointer-events-auto max-w-lg px-6">
        ...
        <div
          ...
          style={{ background: fandomMeta ? fandomMeta.coverColor : `linear-gradient(to bottom right, ${accentColor}, ${accentColor})` }}
        >
        ...
        <h2 className="text-2xl font-black text-foreground mb-2">
          {fandomMeta
            ? `${fandomMeta.groupName} ${templateLabel}를 만들어보세요`
            : "AI로 인스타툰을 만들어보세요"}
        </h2>
        <p className="text-muted-foreground text-sm mb-8">
          ...
          <br />
          {fandomMeta
            ? `AI가 ${fandomMeta.groupName} ${templateLabel}를 생성합니다.`
