    ...
    <div className="space-y-4">
          ...
          <div className="space-y-1">
            {availableSizes.map((sizeKey) => {
              ...
              return (
                ...
                >
                  <span className="text-[13px] font-medium">{size.label}</span>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {([72, 150, 300] as const).map((d) => (
            <button
              key={d}
              onClick={() => setDpi(d)}
              className={`px-2 py-2 rounded-xl border text-center text-[13px] font-medium transition-all ${
                dpi === d
                  ? "bg-primary/10 border-primary/30 text-primary"
                  : "border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.06] text-white/60"
              }`}
            >
              {d}
              <span className="block text-[13px] text-white/30 mt-0.5">
                {d === 72 ? "화면" : d === 150 ? "보통" : "인쇄"}
              </span>
            </button>
          ))}
      ...
      {currentSize && (
        <div className="px-3 py-2 rounded-xl bg-white/[0.02] border border-white/[0.04]">
          <p className="text-[13px] text-white/30">내보내기 시 픽셀 크기</p>
          <p className="text-[13px] text-white/70 font-mono mt-0.5">
      ...
      {/* Guide Toggles */}
      <div className="space-y-1.5">
        <p className="text-[13px] text-white/40 px-1">가이드 표시</p>
        ...
        <label className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/[0.06] bg-white/[0.02] cursor-pointer">
          ...
          <span className="text-[13px] text-white/60">블리드 마크 (3mm)</span>
        </label>
        ...
        <label className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/[0.06] bg-white/[0.02] cursor-pointer">
          ...
          <span className="text-[13px] text-white/60">트림 라인</span>
        </label>
        ...
        <label className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/[0.06] bg-white/[0.02] cursor-pointer">
          ...
          <span className="text-[13px] text-white/60">세이프존 (5mm 안쪽)</span>
        </label>
      </div>
      ...
      <button
        ...
        className="w-full py-2.5 rounded-xl bg-primary/10 border border-primary/20 hover:bg-primary/20 transition-all text-primary text-[13px] font-bold"
      >
        설정 적용
      </button>
    </div>
