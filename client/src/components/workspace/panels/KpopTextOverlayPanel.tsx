  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <p className="text-[11px] text-white/40 px-1">폰트 스타일</p>
        <div className="grid grid-cols-2 gap-1.5">
          {FONT_SUGGESTIONS.map((font) => (
            ...
            >
              <p className={`text-[11px] font-semibold ${selectedFont === font.id ? "text-primary" : "text-white/70"}`}>
                {font.label}
              </p>
              <p className="text-[10px] text-white/30">{font.desc}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        {filteredPresets.map((preset) => (
          ...
          >
            ...
            <span className="text-xs text-white/70">{preset.text}</span>
            <span className="text-[10px] text-white/20 ml-auto">{preset.lang === "ko" ? "한" : "EN"}</span>
          </button>
        ))}
      </div>
      ...
      {/* Custom Text */}
      <div className="space-y-1.5">
        <p className="text-[11px] text-white/40 px-1">직접 입력</p>
        <div className="flex gap-1.5">
          <input
            ...
            className="flex-1 px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-xl text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-primary/40"
            onKeyDown={(e) => e.key === "Enter" && handleCustomText()}
          />
          <button
            onClick={handleCustomText}
            disabled={!customText.trim()}
            className="px-3 py-2 rounded-xl bg-primary/10 border border-primary/20 hover:bg-primary/20 transition-all text-primary text-xs font-bold disabled:opacity-30"
          >
            추가
          </button>
        </div>
      ...
      {palette && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.02] border border-white/[0.04]">
          ...
          <span className="text-[10px] text-white/30">팬덤 컬러 자동 매칭</span>
