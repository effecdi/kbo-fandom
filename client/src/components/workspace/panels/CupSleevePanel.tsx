  ...
  return (
    <div className="space-y-4">
      ...
      <label className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/[0.06] bg-white/[0.02] cursor-pointer">
        ...
        <span className="text-xs text-white/60">접힘선 가이드 표시</span>
      </label>

      {/* Text Areas */}
      <div className="space-y-1.5">
        <p className="text-[11px] text-white/40 px-1">텍스트 영역</p>
        <input
          type="text"
          value={cafeName}
          onChange={(e) => setCafeName(e.target.value)}
          placeholder="카페명 (선택)"
          className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-xl text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-primary/40"
        />
        <input
          type="text"
          value={dateText}
      ...
      <div className="rounded-xl bg-white/[0.02] border border-white/[0.04] p-3 space-y-1">
        <p className="text-[10px] text-white/30">전개도 스펙</p>
