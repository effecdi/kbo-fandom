    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5 px-1">
          ...
          <span className="text-[10px] text-white/40 font-medium">아트 스타일</span>
        </div>
              ...
              {isExpanded && (
                <div className="ml-6 mb-2 space-y-2 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 rounded-lg text-xs font-bold text-white transition-all hover:opacity-90"
                  ...
                  {showPoseOutfit && (
                    <div>
                      <p className="text-[10px] text-white/30 font-medium mb-1 px-1">포즈</p>
                      <div className="flex flex-wrap gap-1">
                        {POSE_CHIPS.slice(0, 4).map((pose) => (
                          <button
                            key={pose}
                            onClick={() => sendMessage(`${member.name} ${pose} 포즈로 그려줘`)}
                            className="px-2 py-1 rounded-md text-[10px] font-medium text-white/50 bg-white/[0.04] hover:bg-white/[0.08] hover:text-white/80 transition-all"
                          >
                            {pose}
                          </button>
                        ))}
                  ...
                  {/* Style options */}
                  <div>
                    <p className="text-[10px] text-white/30 font-medium mb-1 px-1">스타일</p>
                    <div className="flex flex-wrap gap-1">
                  ...
                  {showPoseOutfit && (
                    <div>
                      <p className="text-[10px] text-white/30 font-medium mb-1 px-1">의상</p>
                      <div className="flex flex-wrap gap-1">
